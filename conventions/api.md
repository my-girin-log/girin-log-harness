...
1. 전날 세션들을 합쳐 **Diary 1개**를 자동 생성/정리한다.
2. **채팅 context를 초기화**한다(이후 대화는 새 컨텍스트에서 시작).
3. persona.md / log.md 등 마크다운 문서를 갱신한다.

## 2-1. 대화 세션 라이프사이클 [리뷰 반영]

- 메모(하루 여러 개) → 세션 시작. 하루에 여러 세션 가능.
