...
**Diary 수동 생성 (2026-07-11 결정)**
- 06:00 자동 생성은 기본 경로로 유지한다.
- 사용자는 자동 생성이 누락되었거나 아직 Diary가 없는 **과거 서비스 날짜**에 대해 수동 생성을 요청할 수 있다.
- 수동 생성 대상은 현재 진행 중인 `serviceDate`보다 이전 날짜로 제한한다. 현재 진행 중인 서비스 날짜와 미래 날짜는 아직 하루가 닫히지 않았으므로 `422`로 거부한다.
- 수동 생성도 자동 생성과 같은 입력 규칙을 따른다. 해당 날짜의 Memo 원본과 `ENDED` DailyChatSession을 사용하고, MemoSummary는 있을 때만 보조 힌트로 참고한다.
- 이미 Diary가 있으면 새로 생성하거나 덮어쓰지 않고 기존 Diary를 반환한다(멱등). 대상 날짜에 Memo와 DailyChatSession이 모두 없으면 빈 Diary를 만들지 않고 `422`로 거부한다.
