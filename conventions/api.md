...
2. 전날 **`ENDED` DailyChatSession들**(자동 종료분 포함)을 모아 **Diary 1개**를 자동 생성/정리한다. 원본 Memo·MemoSummary는 Diary 생성의 직접 입력이 아니다.
...
**Diary 생성 대상 (2026-06-09 결정)**
- 전날 `serviceDate`의 `ENDED` DailyChatSession만 입력. (위 1번으로 06:00엔 모두 `ENDED`)
- 그날 DailyChatSession이 **하나도 없으면 Diary를 생성하지 않는다.** 빈 Diary를 만들지 않는다(조회 시 `404` → FE가 "생성 전" 안내).
...
- **MemoSummary는 상태(status)가 없다.** `PENDING`/`USED`/`EXPIRED` 큐 상태를 두지 않는다. 대화에 써도 목록에서 제외하지 않으며(여러 세션이 재사용 가능), "오늘 목록"은 `serviceDate`로 필터링한다. (2026-06-09 결정)

## 2-2. 대화 세션 라이프사이클

- 하나 이상의 MemoSummary 선택 → DailyChatSession 시작. 하루에 여러 세션 가능.
- **한 세션은 하나의 `serviceDate`에만 속한다**(생성 시점 06:00 경계 기준). 선택한 MemoSummary가 서로 다른 `serviceDate`에 걸치면 거부한다(`422`). 시작 시점의 선택 내용은 snapshot으로 보존한다. (2026-06-09 결정)
