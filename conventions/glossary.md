...
| 일일 맥락 스냅샷 | `DailyContext` | Retrospective 생성 내부 입력 | 엔티티/저장 스키마가 아니다. 현재 진행 중인 serviceDate에 아직 Diary가 없을 때만 당일 Memo 원본과 DailyChatSession 원문으로 임시 구성한다 |
...
- "회고를 생성한다" = 선택 기간의 확정 `Diary` + `DailyChatSession` 전체 대화 원문 + `persona.md`로 `Retrospective` 생성. 단, 현재 진행 중인 `serviceDate`에 아직 Diary가 없으면 해당 날짜는 저장하지 않는 임시 `DailyContext`를 사용한다. `DailyContext`는 당일 `Memo` 원본과 `DailyChatSession` 원문으로 만든 생성 시점 스냅샷이며 정식 Diary를 대체하지 않는다. 원본 `Memo`는 Diary 또는 DailyContext에 흡수된 하루 맥락으로 사용하고, `MemoSummary`는 Diary 생성 시 보조 힌트로만 참고한다.
