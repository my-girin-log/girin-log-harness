...
- **06:00 KST에 Memo 원본·DailyChatSession 기반 Diary 자동 생성 + 일일 작업 공간 초기화**. MemoSummary는 있을 때만 보조 힌트로 참고한다. 오늘 회고 생성에는 아직 Diary가 없고 내용 있는 Memo 또는 DailyChatSession이 있는 현재 serviceDate의 기록을 임시 DailyContext로 사용할 수 있다.
...
- Retrospective 생성에 원본 Memo, MemoSummary를 다시 직접 넣기 → 회고 생성 입력은 **Diary 또는 DailyContext + DailyChatSession conversation + persona.md** 다. `DailyContext`는 현재 진행 중인 serviceDate에 아직 Diary가 없고 내용 있는 Memo 또는 DailyChatSession이 있을 때만 만든 임시 입력이다. 기록이 없는 날짜는 건너뛰며, 과거·미래 날짜에 DailyContext를 만들지 않는다.
