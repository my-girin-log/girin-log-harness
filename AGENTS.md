...
- **06:00 KST에 Memo 원본·DailyChatSession 기반 Diary 자동 생성 + 일일 작업 공간 초기화**. MemoSummary는 있을 때만 보조 힌트로 참고한다. 오늘 회고 생성에는 아직 Diary가 없는 현재 serviceDate의 Memo 원본과 DailyChatSession 원문을 임시 DailyContext로 사용할 수 있다.
...
- Retrospective는 선택 기간의 **Diary, DailyChatSession 전체 대화 원문, `persona.md`** 를 기반으로 생성한다. 현재 진행 중인 serviceDate에 아직 Diary가 없으면 저장하지 않는 임시 **DailyContext**를 사용한다. 원본 Memo는 Diary 또는 DailyContext에 흡수된 하루 맥락으로 사용하고, MemoSummary는 Diary 생성 시 보조 힌트로만 참고한다. Retrospective 생성의 기본 직접 입력으로 원본 Memo와 MemoSummary를 다시 넣지 않는다.
...
- Retrospective 생성에 원본 Memo, MemoSummary를 다시 직접 넣기 → 회고 생성 입력은 **Diary 또는 DailyContext + DailyChatSession conversation + persona.md** 다. `DailyContext`는 현재 진행 중인 serviceDate에 아직 Diary가 없을 때만 당일 Memo 원본과 DailyChatSession 원문으로 만든 임시 입력이다.
