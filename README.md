...
- Retrospective는 **선택 기간의 Diary 또는 DailyContext + DailyChatSession 원문 + persona.md** 로 생성한다. 오늘처럼 아직 Diary가 없는 현재 serviceDate는 당일 Memo 원본과 DailyChatSession 원문으로 만든 임시 DailyContext를 사용한다. 원본 Memo는 Diary 또는 DailyContext에 흡수된 하루 맥락으로 사용하고, MemoSummary는 Diary 생성 시 보조 힌트로만 참고한다.
