...
GitHub 잔디처럼 "사용자가 그 날 채팅 세션을 종료했는가"를 날짜 단위로 1개만 기록한다.
누적 카운트가 아니라 `사용자 + 스트릭 날짜` 단위 Boolean 도달 기록이다.

### 동작

- 스트릭 날짜는 DailyChatSession의 서비스 날짜(`serviceDate`, 06:00 KST 경계)와 동일하다.
  - 즉 `2026-06-12 05:59`에 속한 활동은 `2026-06-11` 스트릭, `06:00` 이후는 `2026-06-12` 스트릭.
- DailyChatSession이 종료(ENDED — USER/AI/MAX/SYSTEM 무관)될 때 **백엔드 내부 로직**으로 upsert한다
  (사용자가 직접 누르는 공개 API 아님 → 조작 방지). 종료 트랜잭션과 분리(REQUIRES_NEW)해 기록 실패가 세션 종료를 막지 않는다.
...
| `diaryId` | (선택) 연결된 Diary | nullable. 세션 종료 기준 스트릭에선 보통 비어있음 |
| `completedAt` | 달성 시각(채팅 세션 종료 시각) | |
| `createdAt` | 행 생성 시각 | |
| `updatedAt` | 행 수정 시각 | |
...
DailyChatSession 1 ──> StreakDay  (세션 종료 시 해당 서비스 날짜 스트릭 기록)
...
- `Pet`: 실록이 성장, EXP 등은 MVP 제외. (단, 채팅 세션 종료 기반 streak은 `StreakDay`로 도입됨)
