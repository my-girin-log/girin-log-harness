...
| 5-4 | Diary 수동 생성 | 사용자는 06:00 자동 생성이 누락되었거나 아직 생성되지 않은 과거 서비스 날짜의 Diary 생성을 직접 요청할 수 있어야 한다. 수동 생성은 현재 진행 중인 서비스 날짜와 미래 날짜에는 허용하지 않는다. 이미 Diary가 있는 날짜는 기존 Diary를 반환하는 멱등 동작이어야 하며, 생성 대상 날짜에 Memo와 DailyChatSession이 모두 없으면 빈 Diary를 만들지 않고 거부해야 한다. |
