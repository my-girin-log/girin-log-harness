...
| 2026-06-09 | **회고 생성 입력 = 선택 기간의 `DailyChatSession.conversation` + 원본 `Memo` + `MemoSummary` + `persona.md`.** `Diary`는 직접 입력이 아니다. 기간 기준은 `serviceDate`. | 프론트·BE 합의: 대화 원문만으로는 놓치는 맥락이 있어 원본 기록(Memo)과 카테고리 요약(MemoSummary)까지 함께 넣어 회고 품질을 높이기 위해서. (정리 PR에서 누락됐던 Memo·MemoSummary 포함을 FE 합의로 복원) | `requirements/product.md`, `conventions/api.md`, `domain/data-model.md`, `api/openapi.yaml`, Retrospective 생성 |
