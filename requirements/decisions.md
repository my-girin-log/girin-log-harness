...
| 2026-06-09 | **회고 생성 입력 = 선택 기간의 `DailyChatSession.conversation` + `persona.md`.** 원본 `Memo`, `MemoSummary`, `Diary`는 직접 입력이 아니다. 기간 기준은 `serviceDate`. | 회고 생성은 기간 내 대화 원문과 사용자다운 말투 기준을 결합하는 흐름으로 단순화하고, Diary는 날짜별 조회용 요약으로 역할을 분리하기 위해서. | `requirements/product.md`, `conventions/api.md`, `domain/data-model.md`, `api/openapi.yaml`, Retrospective 생성 |
...
| 2026-06-10 | **온보딩 재제출을 허용한다.** 재제출 시 Persona는 사용자당 1개를 **덮어쓰기(갱신)**, 별도 버전 없음. `OnboardingSurvey`·`PersonaSource`는 누적. 차단(`409`/`403`) 없음, `onboardingCompleted`는 true 유지. | 재온보딩을 막기보다 갱신이 자연스럽고(Persona 주기 갱신 방향과 일치), 버전 관리는 MVP에 과하기 때문. | `conventions/api.md`, `domain/data-model.md`, Onboarding 구현 |
